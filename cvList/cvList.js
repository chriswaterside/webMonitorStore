class cvList {
    ...
    createPagination(paginationOptions, itemsPerPage) {
        this._pagination = new cvListPagination(this._displayTag, paginationOptions, itemsPerPage);
...
// internal functions
    ...
    _display() {
        ...
        for (var key in keys) {
            ...
            this._displayTag.appendChild(this._items[itemKey]);
        }
        if (keys.length === 0) {
    ...
    _addField(title, type) {
        ...
        switch (type.toLowerCase()) {
            ...
            case 'date':
                ...
                break;
            default:
                alert('Invalid type of cvList Field defined');
                return null;
        }
...
class cvListField {
    ...
    _getItemValue(attrib, item) {
        ...
        const matches = item.querySelectorAll(search);
        if (matches.length > 0) {
...
class cvListFieldNumber extends cvListField {
    ...
    _display() {
        ...
        title.innerText = this.title;
        div.appendChild(title);
        var minText = this._addElement(div, 'span', 'minText');
        minText.innerText = this.minValue;
        div.appendChild(minText);
        var slider = createMinMaxSlider(this.minValue, this.maxValue);
        div.appendChild(slider);
        var maxText = this._addElement(div, 'span', 'maxText');
        maxText.innerText = this.maxValue;
        div.appendChild(maxText);
        slider.addEventListener("minmaxSlider-change", function (e) {
            clearTimeout(mouseStopTimer);
            self.filterValue = e.range;
            minText.innerText = e.range.min;
            maxText.innerText = e.range.max;
            mouseStopTimer = setTimeout(function () {
...
class cvListFieldDate extends cvListField {
    ...
    _sortKeys(keys, items, order) {
        ...
        for (var key of keys) {
            var item = items[key];
            var text = super._getItemValue('data-cvListDateField', item);
            if (text === null) {
                values.push[{key: key, value: text}];
            } else {
                values.push({key: key, value: text});
...
class cvListPagination {
    constructor(displayTag, paginationOptions, itemsPerPage) {
...
class cvListPaginationDisplayItem {
    ...
    _displayInfo(data) {
        ...
        //  {pageNumber}, {pagesNumber}, {startItem}, {endItem}, {itemsNumber}
    ...
    _createPaginationButtons(tag, data) {
        ...
        last = current + 4;
        if (last > data.pagesNumber) {
            last = data.pagesNumber;
        }
        previous = current - 1;
        if (previous < 1) {
            previous = 1;
        ...
        next = current + 1;
        if (next > data.pagesNumber) {
            next = data.pagesNumber;
        }
        ...
        this._addPageButton(tag, next, '&gt;', data.pagesNumber !== current, false);
        this._addPageButton(tag, data.pagesNumber, '&gt;|', data.pagesNumber !== current, false);
    }
    ...
    _addPageItemInfo(content, data) {
        ...
        content = content.replaceAll("{pagesNumber}", data.pagesNumber);
...
createMinMaxSlider = function (minValue, maxValue) {
    var range = {min: minValue,
        max: maxValue};
    var options = {
        width: 200,
        thumbnailWidth: 16
    ...
    // options = setOptions(options, userOptions);
    slider = document.createElement('div');
    slider.classList.add("min-max-slider");
    minSlider = createRangeInput('min', minValue, maxValue);
    slider.appendChild(minSlider);
    maxSlider = createRangeInput('max', minValue, maxValue);
    ...
    minSlider.addEventListener("input", function () {
        minMaxSliderDraw();
        minSlider.setAttribute('title', 'min value: ' + minSlider.value);
        let event = new Event("minmaxSlider-change");
        event.range = {min: minSlider.value,
            max: maxSlider.value};
        slider.dispatchEvent(event);
    });
    maxSlider.addEventListener("input", function () {
        minMaxSliderDraw();
        maxSlider.setAttribute('title', 'Max value: ' + maxSlider.value);
        let event = new Event("minmaxSlider-change");
        event.range = {min: minSlider.value,
            max: maxSlider.value};
        slider.dispatchEvent(event);
    ...
    return slider;
    function createRangeInput(id, minValue, maxValue) {
        var ele = document.createElement('input');
        ele.classList.add('min', 'minmaxslider');
        ...
        if (id === 'min') {
            ele.setAttribute('value', minValue);
            ele.setAttribute('title', 'Min value: ' + minValue);
        }
        if (id === 'max') {
            ele.setAttribute('value', maxValue);
            ele.setAttribute('title', 'Max value: ' + maxValue);
        }
        return ele;
    }
//    function setOptions() {
...
//    }
    ...
    function minMaxSliderDraw() {
        ...
        //   var minWidth = parseInt(thumbsize + ((splitvalue - range.min) / (range.max - range.min)) * (sliderwidth - (2 * thumbsize)));
        //    var maxWidth = parseInt(thumbsize + ((range.max - splitvalue) / (range.max - range.min)) * (sliderwidth - (2 * thumbsize)));

        var maxWidth = sliderwidth - minWidth;
        /* set min and max attributes */
        minSlider.setAttribute('min', range.min);
        minSlider.setAttribute('max', splitvalue);
        maxSlider.setAttribute('min', splitvalue);
        maxSlider.setAttribute('max', range.max);
